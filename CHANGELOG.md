...
- Require `webpatser/resonate` `^0.7` and `webpatser/resonate-roster` `^0.7` (was `^0.4|^0.5|^0.6|^0.7` and `^0.3.1|^0.4`), dropping support for resonate 0.6 and earlier.
...
- Support `webpatser/resonate` v0.7. With scaling enabled, presence subscribe and close hooks can land up to ~1s later, because core now queries other nodes first before firing them, so `onSubscribe` and the departure path read occupancy that lags by roughly that much.
